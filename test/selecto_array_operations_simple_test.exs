defmodule SelectoArrayOperationsSimpleTest do
  ...
  alias Selecto.Builder.ArrayOperations, as: ArrayBuilder
  ...
  describe "Array Operations Specification" do
    ...
    test "creates array_agg with DISTINCT" do
      spec = ArrayOperations.create_array_operation(:array_agg, "rating", 
        distinct: true, 
        as: "unique_ratings"
      )
    ...
    test "creates array_agg with ORDER BY" do
      spec = ArrayOperations.create_array_operation(:array_agg, "title",
    ...
    test "creates string_agg specification" do
      spec = ArrayOperations.create_array_operation(:string_agg, "name",
        delimiter: ", ",
        as: "names_list"
      )
  ...
  describe "SQL Generation" do
    ...
    test "generates array overlap SQL" do
      spec = ArrayOperations.create_array_filter(:array_overlap, "categories", ["tech", "science"])
      {sql, params} = ArrayOperations.to_sql(spec, [])
    ...
    test "generates array contained SQL" do
      spec = ArrayOperations.create_array_filter(:array_contained, "permissions", ["read", "write", "admin"])
      {sql, params} = ArrayOperations.to_sql(spec, [])
    ...
    test "generates ARRAY_APPEND SQL" do
      spec = ArrayOperations.create_array_operation(:array_append, "tags",
        value: "new-tag",
        as: "updated_tags"
      )
      {sql, params} = ArrayOperations.to_sql(spec, [])
    ...
    test "generates ARRAY_REMOVE SQL" do
      spec = ArrayOperations.create_array_operation(:array_remove, "tags",
        value: "deprecated",
        as: "cleaned_tags"
      )
      {sql, params} = ArrayOperations.to_sql(spec, [])
    ...
    test "generates ARRAY_TO_STRING SQL" do
      spec = ArrayOperations.create_array_operation(:array_to_string, "tags",
        value: ", ",
        as: "tag_string"
      )
      {sql, params} = ArrayOperations.to_sql(spec, [])
