defmodule SelectoEdgeCasesTest do
  ...
  describe "Empty Result Sets" do
    test "filter that matches no records", %{actor_selecto: selecto} do
      ...
      |> Selecto.filter({"actor_id", -1})  # Non-existent ID
      |> Selecto.execute()
    ...
    test "impossible range filter", %{film_selecto: selecto} do
      ...
      |> Selecto.filter({"film_id", {">", 99999}})  # Beyond max film ID
      |> Selecto.execute()
    ...
    test "string filter that matches nothing", %{actor_selecto: selecto} do
      ...
      |> Selecto.filter({"first_name", "XXXXXXXXXX"})  # Non-existent name
      |> Selecto.execute()
    ...
    test "LIKE pattern that matches nothing", %{actor_selecto: selecto} do
      result = selecto
      |> Selecto.select(["first_name"])
      |> Selecto.filter({"first_name", {:like, "XYZ%"}})  # No names start with XYZ
      |> Selecto.execute()
  ...
  describe "NULL Value Handling" do
    ...
    test "IS NOT NULL filter", %{film_selecto: selecto} do
      ...
      assert {:ok, {rows, _columns, _aliases}} = result
      assert length(rows) > 0  # Most/all films have titles
    ...
    test "NULL in aggregation functions", %{film_selecto: selecto} do
      ...
      |> Selecto.select([
        {:count, "*"},
        {:count, "length"},  # Count non-NULL length values
        {:avg, "length"},    # Average of non-NULL length values
        {:min, "length"},
        {:max, "length"}
      ])
      |> Selecto.execute()
      ...
      assert length_count <= total_count  # Some films might have NULL length
    ...
    test "COALESCE with NULL values", %{film_selecto: selecto} do
      ...
      |> Selecto.select([
        "title",
        {:coalesce, ["length", {:literal, 0}]}  # Replace NULL length with 0
      ])
      ...
      Enum.each(rows, fn [title, coalesced_length] ->
        ...
        assert coalesced_length >= 0  # Should be actual length or 0
      end)
    end
  end
  ...
  describe "Type Conversion Edge Cases" do
    ...
    test "boolean-like string handling", %{film_selecto: selecto} do
      ...
      |> Selecto.filter({"title", "true"})  # Treating string field as boolean
      |> Selecto.execute()
  ...
  describe "Large Data Edge Cases" do
    test "very large IN list", %{actor_selecto: selecto} do
      ...
      large_list = Enum.to_list(1..200)  # All possible actor IDs
      ...
      assert {:ok, {rows, _columns, _aliases}} = result
      assert length(rows) == 200  # All actors
    ...
    test "extremely large IN list", %{actor_selecto: selecto} do
      ...
      assert {:ok, {rows, _columns, _aliases}} = result
      assert length(rows) <= 200  # Only actual actors
    end

    test "select all fields with large result set", %{film_selecto: selecto} do
      result = selecto
      |> Selecto.select(["film_id", "title", "description", "release_year", "rental_rate", "length", "rating"])
      |> Selecto.execute()
  ...
  describe "Invalid Input Handling" do
    ...
    test "malformed filter structure", %{actor_selecto: selecto} do
      ...
      malformed_filters = [
        {"actor_id"},  # Missing value
        {"actor_id", "value", "extra"},  # Too many elements
        {"actor_id", {"incomplete_tuple"}}  # Malformed operator tuple
      ]
  ...
  describe "SQL Injection Prevention" do
    test "SQL injection in string filters", %{actor_selecto: selecto} do
      ...
      Enum.each(injection_attempts, fn injection_string ->
        ...
        assert {:ok, {rows, _columns, _aliases}} = result
        assert length(rows) == 0  # No actors with these malicious names
      end)
    end
  ...
  describe "Concurrent Access Edge Cases" do
    test "multiple simultaneous queries", %{actor_selecto: selecto} do
      ...
      Enum.each(results, fn result ->
        assert {:ok, {rows, _columns, _aliases}} = result
        assert length(rows) == 1  # Each query for a single actor
      end)
    end
  ...
  describe "Boundary Value Testing" do
    test "minimum and maximum integer values", %{actor_selecto: selecto} do
      ...
      boundary_tests = [
        {{"actor_id", {">", 0}}, fn count -> count == 200 end},  # All actors
        {{"actor_id", {"<", 1}}, fn count -> count == 0 end},   # No actors
        {{"actor_id", {">=", 200}}, fn count -> count == 1 end}, # Just actor 200
        {{"actor_id", {"<=", 1}}, fn count -> count == 1 end}    # Just actor 1
      ]
